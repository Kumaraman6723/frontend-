...
export default function Found({ darkMode }) {
  ...
  const logEndpoint = getLogsUrl("USER_LOGS");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      if (isAdmin) {
        await axios.delete(`${getApiUrl("/api/reports")}/${id}`, {
    ...
    try {
      ...
      if (isAdmin) {
        response = await axios.put(
          `${getApiUrl("/api/reports")}/${currentItem._id}`,
          updatedData,
          {
            headers: {
              Email: user.email,
  ...
  //       `http://localhost:8000/api/reports/${modalItem._id}/claim`,
  //       { proofDescription },
  //       {
  //         headers: { Email: user.email },
  //       }
  ...
  const handleSubmitProof = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${getApiUrl("/api/reports")}/${modalItem._id}/claim`,
        { proofDescription },
        {
          headers: { Email: user.email },
        }
