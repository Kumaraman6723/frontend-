...
export default function Section1({ darkMode }) {
  ...
  useEffect(() => {
    axios
      .get(getApiUrl("/api/reports"))
      .then((response) => {
        const filteredReports = response.data.filter(
          (report) => report.reportType === reportType && report.claimedBy
        );
  ...
  const handleReset = (reportId) => {
    ...
    axios
      .put(
        `${getApiUrl("/api/reports")}/${reportId}/reset`,
        {
  ...
  const handleVerify = async (reportId) => {
    try {
      ...
      await axios.put(`${getApiUrl("/api/reports")}/${reportId}/verify`, {
        otp: otpInput,
      });
  ...
  const handleSendOtp = async (reportId) => {
    try {
      ...
      const response = await axios.put(
        `${getApiUrl("/api/reports")}/${reportId}/send-otp`,
        null,
        {
          headers: {
      ...
      {!loading && (
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-4">
            Claimed Items Verification
          </h1>
          <p className="text-lg text-gray-600 mb-4">
            View and verify all claimed items. Verified items are marked with a
            green badge.
          </p>
          <div className="flex justify-center space-x-8 mb-4">
            ...
            <div className="bg-yellow-100 text-yellow-800 px-4 py-2 rounded-lg">
              <span className="font-semibold">
                ...
                {
                  reports.filter(
                    (r) => r.verificationStatus === "Under Verification"
                  ).length
                }
              </span>
            </div>
